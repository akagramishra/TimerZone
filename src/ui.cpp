...
namespace UI
{
  ...
  void DrawTimerPanel(POMODORO_TIMER &timer, Rectangle bounds)
  {
    ...
    const char *timeText;
    timeText = TextFormat("%02d:%02d:%02d", hour, minute, second); //consistent display text
    ...
    // progress can't go below 0 or above one, 0<=timer.timeleft<=totalTime
    //  if (progress < 0.0f)
    //    progress = 0.0f;
    //  if (progress > 1.0f)
    //    progress = 1.0f;
    ...
    Rectangle btn1 = {bounds.x + 120, btnY, btnW, btnH};
    ...
    if(!timer.isRunning)
      ...
      DrawModernButton(btn1, "|| PAUSE", ButtonDarkColor, {60, 68, 80, 255},
                     SecondaryAccent, clicked1);
    DrawModernButton(btn2, ">> RESET", ButtonDarkColor, {60, 68, 80, 255},
                     TextMutedColor, clicked2);
